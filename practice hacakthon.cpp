...
void loadFromFile() {
    ...
    while (fscanf(fp, "%d\n", &students[count].roll) != EOF) {
        fgets(students[count].name, 50, fp);
        students[count].name[strcspn(students[count].name, "\n")] = 0;
        for (int j = 0; j < SUBJECTS; j++)
            fscanf(fp, "%f", &students[count].marks[j]);
        fscanf(fp, "\n");
        fscanf(fp, "%f %f %c\n", &students[count].total, &students[count].percentage, &students[count].grade);
        count++;
        if (count >= MAX) break;
    }
    ...
    while (1) {
        ...
        for (int i = 0; i < count; i++) {
            if (students[i].roll == s.roll) {
                printf("This Roll Number already exists! Enter a different one.\n");
                duplicate = 1;
                break;
            }
        }
